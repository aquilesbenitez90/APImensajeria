...
- LARGO (CRÍTICO — la página 1 tiene que RESPIRAR, no ser un muro de texto; pero el "lead" SÍ desarrolla): el "lead" es la APERTURA del documento, 3 oraciones cortas y PLANAS (humanas, no corporativas): presenta qué hace el cliente, enmarca que armamos este diagnóstico para que sepa cuándo y cómo contactar a sus clientes potenciales, y dice que le compartimos ${N} encontrados con nuestra tecnología con el mensaje listo. No es telegrama pero TAMPOCO un párrafo denso; SIN nombres de marcas ni jerga en la apertura. TODO LO DEMÁS corto: proof = 1 oración; cada desc del ICP = FRASE TELEGRÁFICA de máx ~14 palabras (frase nominal, sin subordinadas, sin relleno: ej. "VP/Director de Operaciones o RobOps Lead", NO un párrafo); "Señal de compra" y "Pain primario" = media oración cada una (máx ~14 palabras); cada bullet de context = máx ~16 palabras; cada prioridad = "Alta/Media:" + frase de máx ~12 palabras. Fuera del lead, cortá toda palabra que no aporte y preferí frases nominales. El objetivo: el lead explica y vende; el resto se lee de un vistazo.
...
## Output — SOLO JSON (sin texto ni markdown alrededor)
{
  ...
  "lead": "APERTURA del documento (lo PRIMERO que lee el cliente), 3 oraciones cortas, PLANA y HUMANA (como le hablás a un cliente real, NADA corporativo ni jerga). Estructura: (1) '[Cliente] es una empresa que [qué hace, concreto y claro, SIN nombres de marcas/clientes ni tecnicismos]'; (2) 'Sobre esa base armamos este diagnóstico, para que sepa cuándo y cómo contactar a sus clientes potenciales'; (3) 'Le compartimos ${N} de los que encontramos con nuestra tecnología, con el mensaje listo para escribirles'. Redactá (2) y (3) naturales pero SIN cambiar el número ${N} ni inventar métricas. Trato de usted.",
