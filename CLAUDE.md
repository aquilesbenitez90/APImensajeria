...
| GET | `/leaderboard` | Ranking **mensual** de quién generó cuántos diagnósticos (campo `generado_por` de `resultados.jsonl` del volumen, NO del Sheet; solo filas con usuario). **Conteo**: 1 por EMPRESA distinta por persona (`_leadKeyDiag`: repetir la misma empresa no suma, va a `repetidos`), los `status:error` no cuentan, y `LEADERBOARD_DESDE` (env ISO) es piso absoluto (reset del ranking; hoy `2026-09-18T03:00:00Z`). La landing manda `?desde=<día 1 del mes 00:00 local ISO>` (arranca de cero cada mes); alternativa `?dias=N` (default 30, 0 = todo). Gate: token de Google con email permitido (modo Google) o `x-landing-key`/`STATS_KEY`. La landing lo muestra como panel lateral derecho con sesión (solo posición, quién y cantidad; aprobados/tasa viajan en el JSON pero NO se muestran, decisión del negocio). |

Gate de `/generar`, `/generar-reporte` y `/leaderboard` (`_gateGenerar`): con `GOOGLE_CLIENT_ID` seteado → login con Google obligatorio. El front canjea el ID token de Google (vence a la HORA) en `POST /sesion` por una **sesión propia** firmada HMAC (`ibt.<payload>.<firma>`, `SESSION_DIAS=30`, secreto `SESSION_SECRET`; sin él se genera al arrancar y las sesiones mueren en cada deploy). `x-user-token` acepta las dos cosas (sesión o ID token crudo); `ALLOWED_EMAILS` se re-chequea en cada request. La clave vieja NO vale acá, si no nadie se loguearía; solo `LANDING_KEY` → clave compartida; ninguna → sin gate (producción/n8n). Quién generó queda como `generado_por` en `resultados.jsonl`, columna `usuario` en el Sheet y columna "Generó" en `/pdfs`.
...
Leaderboard: `LEADERBOARD_DESDE` (ISO; nada anterior cuenta, sirve para resetear el ranking).
Sesión de la landing: `SESSION_SECRET` (hex random; seteado en Railway), `SESSION_DIAS=30`.
